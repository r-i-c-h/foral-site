...
pageIndex: 1
slugPath: /pathways-to-graduation-and-beyond
heroImage: ./a1-throw-hats.jpg
heroAlt: School Graduation
seoTitle: Pathways to Graduation and Beyond
---
...
To use a page's **FRONTMATTER** from _within_ the same page: Docs say call to `props.pageContext.frontmatter.____`, but that didn't work. Maybe because I have an extra template rendering layer? Using the "secret" `layoutProps._frontmatter.heroImagePath` _did_ work for me, but now I've moved the graphql query into the post-layout.js page and pass down `front` and `site` as props through `<MDXRenderer>`... &lt;shruggo&gt;
